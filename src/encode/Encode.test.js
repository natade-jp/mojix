...

{
	...
	test("encode 1-1", () => {
		expect(equalsArray(Encode.encode(text, "utf-8"), utf_8_bom)).toBe(true);
	});
	test("encode 1-2", () => {
		expect(equalsArray(Encode.encode(text, "utf-8", false), utf_8)).toBe(true);
	});
	test("encode 1-3", () => {
		expect(equalsArray(Encode.encode(text, "utf-16le", false), utf_16le)).toBe(true);
	});
	test("encode 1-4", () => {
		expect(equalsArray(Encode.encode(text, "utf-16be", false), utf_16be)).toBe(true);
	});
	test("encode 1-5", () => {
		expect(equalsArray(Encode.encode(text, "utf-32le", false), utf_32le)).toBe(true);
	});
	test("encode 1-6", () => {
		expect(equalsArray(Encode.encode(text, "utf-32be", false), utf_32be)).toBe(true);
	});

	test("decode 1-1", () => {
		expect(Encode.decode(utf_8_bom)).toBe(text);
	});
	test("decode 1-2", () => {
		expect(Encode.decode(utf_16be_bom)).toBe(text);
	});
	test("decode 1-3", () => {
		expect(Encode.decode(utf_16le_bom)).toBe(text);
	});
	test("decode 1-4", () => {
		expect(Encode.decode(utf_32be_bom)).toBe(text);
	});
	test("decode 1-5", () => {
		expect(Encode.decode(utf_32le_bom)).toBe(text);
	});
	test("decode 1-6", () => {
		expect(Encode.decode([0x61, 0x62])).toBe("ab");
	});
	test("decode 1-7", () => {
		expect(Encode.decode(utf_16be, "utf-16be")).toBe(text);
	});
}

{
	...
	test("encode 2-1", () => {
		expect(equalsArray(Encode.encode(text, "Windows-31J"), cp932)).toBe(true);
	});
	test("decode 2-1", () => {
		expect(Encode.decode(cp932, "Windows-31J")).toBe(text);
	});
	test("decode 2-2", () => {
		expect(Encode.decode(cp932)).toBe(text);
	});
}

{
	...
	test("encode 3-1", () => {
		expect(equalsArray(Encode.encode(text, "SJIS-2004"), sjis2004)).toBe(true);
	});
	test("decode 3-1", () => {
		expect(Encode.decode(sjis2004, "SJIS-2004")).toBe(text);
	});
}

{
	...
	test("encode 4-1", () => {
		expect(equalsArray(Encode.encode(text, "eucJP-ms"), eucjp)).toBe(true);
	});
	test("decode 4-1", () => {
		expect(Encode.decode(eucjp, "eucJP-ms")).toBe(text);
	});
}

{
	...
	test("encode 5-1", () => {
		expect(equalsArray(Encode.encode(text, "EUC-JP"), eucjp)).toBe(true);
	});
	test("decode 5-1", () => {
		expect(Encode.decode(eucjp, "EUC-JP")).toBe(text);
	});
}

{
	...
	test("encode 6-1", () => {
		expect(equalsArray(Encode.encode(text, "EUC-JIS-2004"), eucjis2004)).toBe(true);
	});
	test("decode 6-1", () => {
		expect(Encode.decode(eucjis2004, "EUC-JIS-2004")).toBe(text);
	});
	test("decode 6-2", () => {
		expect(Encode.decode(eucjis2004)).toBe(text);
	});
}
